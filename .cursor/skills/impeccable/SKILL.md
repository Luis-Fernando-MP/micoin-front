...
4. When creating or extracting reusable UI, load [reference/component-black-box.md](reference/component-black-box.md) and `AGENTS.md`. Pieces live in `src/common/components/{name}` with `export default`, Spanish JSDoc, and `BRAND` shared props. Extra capability goes in `extensions/{domain}`, never in a fat core.
