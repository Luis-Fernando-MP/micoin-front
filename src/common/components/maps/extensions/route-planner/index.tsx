  ...
  return (
    <View className="gap-1.5">
      ...
      {open && value.trim().length >= 2 && (
        <View className={cn('border border-border bg-card', BRAND.radius.variants.control)}>
          {loading && (
...
const RoutePlanner: FC<Props> = ({
  ...
  const [queryB, setQueryB] = useState((destination ?? DEFAULT_DESTINATION).label);
  const [placeA, setPlaceA] = useState<PlaceOption | null>(origin ?? DEFAULT_ORIGIN);
